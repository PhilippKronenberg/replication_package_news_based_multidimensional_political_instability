function vm_plot_pe_bvar(mmodel,identification,fflagFEVD,nper)
...
nv = size(VAR.LtildeFullPE,1);
Horizon = size(VAR.LtildeFullPE,2);
H = Horizon -1;
nshock = 2; 
selMatrix = 1:nv;
...
%     fig = figure;
    for ii = 1:nsel % Variable
        subplot(nr,nc,ii)
        plot(0:1:H,squeeze(VAR.LtildeFullPE(selMatrix(ii),1:Horizon,3,jj)),'LineWidth',2)     
        hold on
        plot(0:1:H,squeeze(VAR.LtildeFullPE(selMatrix(ii),1:Horizon,2,jj)),'r--', ...
             'LineWidth',2)
        plot(0:1:H,squeeze(VAR.LtildeFullPE(selMatrix(ii),1:Horizon,4,jj)),'r--', ...
             'LineWidth',2)
        plot(0:1:H,squeeze(VAR.LtildeFullPE(selMatrix(ii),1:Horizon,1,jj)),'g--', ...
             'LineWidth',2)
        plot(0:1:H,squeeze(VAR.LtildeFullPE(selMatrix(ii),1:Horizon,5,jj)),'g--', ...
             'LineWidth',2)
         axis([0 H ylim])     
...
if pprint == 1
    ...
    print(fig,'-dpdf',fullfile(resultsDir, strcat('PE_',char(mmodel),'shock_',num2str(jj))));
end
